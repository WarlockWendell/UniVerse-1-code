...
try:
    from fastvideo.models.music_dcae.music_vocoder import ADaMoSHiFiGANV1
except ImportError:
...
class MusicDCAE(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        self.resampler = torchaudio.transforms.Resample(source_sample_rate, self.target_sample_rate)
        ...
        self.audio_chunk_size = int(round((1024 * 512 / self.target_sample_rate * 48000)))
    ...
    def encode(self, audios, audio_lengths=None, sr=None):
        ...
        if sr is None:
            ...
            resampler = torchaudio.transforms.Resample(sr, self.target_sample_rate).to(device).to(dtype)
        ...
        if max_audio_len % (8 * 512) != 0:
            ...
            # )
            audio = torch.cat([audio[:, :, :512*2].repeat(1, 1, 3), audio], -1)
        ...
        latent_lengths = (
            audio_lengths / sr * self.target_sample_rate / 512 / self.time_dimention_multiple
        ).long()
        latents = (latents - self.shift_factor) * self.scale_factor
        return latents, latent_lengths, mels
    ...
    def decode(self, latents, audio_lengths=None, sr=None):
        ...
        for latent in latents:
            ...
            if sr is not None:
                resampler = (
                    torchaudio.transforms.Resample(self.target_sample_rate, sr)
                )
    ...
    def decode_overlap(self, latents, audio_lengths=None, sr=None):
        ...
        MODEL_INTERNAL_SR = self.target_sample_rate
    ...
    def forward(self, audios, audio_lengths=None, sr=None):
        latents, latent_lengths, mels = self.encode(
        ...
        return sr, pred_wavs, latents, latent_lengths, mels
